...
switch ($_GET["op"]){
	case 'guardaryeditar':
		...
		if (empty($idlibro)){
			$rspta=$libro->insertar($titulo,$cantidad_disponible,$idautor,$year_edicion,$numero_paginas,$formato,$peso,$descripcion,$imagen);
			echo $rspta ? "Libro registrado" : "Libro no se pudo registrar";
		}
		else {
			$rspta=$libro->editar($idlibro,$titulo,$cantidad_disponible,$idautor,$year_edicion,$numero_paginas,$formato,$peso,$descripcion,$imagen);
